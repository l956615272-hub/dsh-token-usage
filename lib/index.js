...
/**
 * The provider directory exactly as the Models settings page builds it:
 * configurable providers declared by adapter plugins plus registered adapters,
 * joined by provider id. This is the source of truth for which access points
 * exist; anything absent here never appears in the quota card.
 *
  ...
  try {
    ...
    if (typeof llm.listConfigurableProviders === 'function') {
      for (const entry of llm.listConfigurableProviders() || []) {
        if (entry && typeof entry.provider === 'string' && entry.provider.length > 0) {
          names.set(entry.provider, typeof entry.displayName === 'string' && entry.displayName.length > 0 ? entry.displayName : entry.provider);
        }
      }
    }
    for (const provider of llm.listProviders() || []) {
      if (provider && typeof provider.id === 'string' && provider.id.length > 0 && !names.has(provider.id)) {
