...
import { buildSummary, clampDay, dayKey, filterRecords, resolveRange, summarize } from './core/aggregate.js';
...
function readQuotaMap(file) {
  ...
  if (raw.providers && typeof raw.providers === 'object' && !Array.isArray(raw.providers)) {
    ...
    for (const [provider, entry] of Object.entries(raw.providers)) {
      ...
      const normalized = normalizeQuotaEntry(entry);
      // Drop empty placeholders (the earlier UI wrote one per discovered
      // provider); an entry survives only once it was actually filled in.
      if (!normalized.enabled && normalized.amount <= 0 && normalized.label.length === 0) continue;
...
/**
 * The access points the Models settings page actually shows as added: the
 * adapters registered with the live LLM registry. The page renders
 * listConfigurableProviders() filtered by whether each provider is configured,
 * and the configured ones are exactly the registered routes; the configurable
 * directory alone is the whole preset catalog (anthropic, groq, ...), which
 * must never leak into the quota card.
 *
 * Returns a Map of provider id to display name, or null when the live LLM
 * registry is unavailable (the caller then falls back to file/usage discovery).
 */
function readModelProviders(ctx) {
  ...
  try {
    ...
    if (typeof llm.listConfigurableProviders === 'function') {
      for (const entry of llm.listConfigurableProviders() || []) {
        if (entry && typeof entry.provider === 'string' && entry.provider.length > 0) declared.add(entry.provider);
      }
    }
    const names = new Map();
    for (const provider of llm.listProviders() || []) {
      if (!provider || typeof provider.id !== 'string' || provider.id.length === 0) continue;
      // Registered but outside the declared directory is not an "added" row on
      // the Models page either, so drop it here as well.
      if (declared.size > 0 && !declared.has(provider.id)) continue;
      names.set(provider.id, typeof provider.name === 'string' && provider.name.length > 0 ? provider.name : provider.id);
    }
...
export function apply(ctx, config) {
  ...
  const buildQuotaPayload = () => {
    ...
    for (const record of records) {
      ...
      if (time > (lastTime.get(provider) || 0)) lastTime.set(provider, time);
    }

    const modelProviders = readModelProviders(ctx);
    ...
    if (quotaMap.__legacy && Object.keys(quotaMap).length === 1) {
      ...
      for (const [provider, total] of totals) {
        ...
        if (modelProviders && !modelProviders.has(provider)) continue;
    ...
    const known = new Set();
    if (modelProviders) {
      // Source of truth: the same provider directory the Models page renders.
      for (const provider of modelProviders.keys()) known.add(provider);
    } else {
    ...
    for (const provider of known) {
      const entry = quotaMap[provider];
      const displayName = modelProviders && modelProviders.get(provider) ? modelProviders.get(provider) : provider;
      if (!entry) {
        // Declared by the Models page but without a quota yet: offered in the
        // add picker, never rendered as its own card row.
        const last = lastTime.get(provider);
        available.push({
          provider,
          label: displayName,
          total: totals.get(provider) || 0,
          lastDay: last ? dayKey(last, store.timeZone) : null
        });
        continue;
      }
    ...
    const byTotal = (a, b) => b.total - a.total || (a.provider < b.provider ? -1 : 1);
    providers.sort(byTotal);
    available.sort(byTotal);
