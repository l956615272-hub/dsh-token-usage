...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function quotaRow(item, form, onChange) {
      ...
      const showFields = enabled || Number(form.amount) > 0;
      return h('div', { key: item.provider, style: styles.quotaRow },
        h('div', { style: styles.quotaRowHead },
          ...
          h('div', { style: styles.quotaCycle }, cycleText)
        ),
        configured
          ? h('div', null,
    ...
    function TokenUsageSection() {
      ...
      React.useEffect(() => {
        if (!quotaReady && !quotaDirty && quota && Array.isArray(quota.providers)) {
          const form = {};
      ...
      const quotaPanel = h('div', { style: styles.quota },
        h('div', { style: styles.quotaHead },
          ...
          h('div', { style: styles.quotaCycle }, '按接入方分别配置；关闭开关的接入方只统计用量，不算剩余')
        ),
        providers.length === 0
          ? h('div', { style: styles.quotaHint }, quota ? '暂无接入方数据。' : '正在加载额度配置…')
          : h('div', null, providers.map((item) => quotaRow(item, entryOf(item.provider), (patch) => editQuotaProvider(item.provider, patch)))),
        providers.length > 0
