...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styles = {
      ...
      quotaRemainRow: { display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 8, flexWrap: 'wrap' },
      quotaRemainValue: { fontSize: 26, fontWeight: 600, fontVariantNumeric: 'tabular-nums' },
      quotaRemainMeta: { fontSize: 12, opacity: 0.65 },
    ...
    function TokenUsageSection() {
      ...
      React.useEffect(() => {
        if (!quotaReady && quota && quota.config) {
          setQuotaForm({
            amount: quota.config.amount ? String(quota.config.amount) : '',
      ...
      const saveQuota = () => {
        ...
        fetch('/dsh-token-usage/quota', {
          ...
          body: JSON.stringify({
            amount: Number(quotaForm.amount) || 0,
            unit: quotaForm.unit,
            refreshDay: Number(quotaForm.refreshDay) || 1,
      ...
      const editQuota = (patch) => setQuotaForm(Object.assign({}, quotaForm, patch));
      ...
      const quotaPanel = h('div', { style: styles.quota },
        ...
        configured
          ? h('div', null,
              h('div', { style: styles.quotaRemainRow },
                h('div', { style: styles.quotaRemainValue }, formatUnits(quota.remaining)),
                h('div', { style: styles.quotaRemainMeta }, '剩余 · 额度 ' + formatUnits(quota.quotaTokens) + '（' + formatNumber(quota.quotaTokens) + '）')
              ),
              ...
              h('div', { style: styles.quotaStats },
                h('span', null, '已用 ' + formatUnits(quota.used) + '（' + (ratio * 100).toFixed(1) + '%）'),
          ...
          : h('div', { style: styles.quotaHint }, '填写本月额度后，这里会显示本周期剩余。配置只保存在本机（$DSH_HOME/dsh-token-usage/quota.json），不会上传。'),
