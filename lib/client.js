...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styles = {
      ...
      quotaRemainLabel: { fontSize: 11, opacity: 0.6, marginTop: 10 },
      quotaRemainRow: { display: 'flex', alignItems: 'baseline', gap: 12, marginTop: 2, flexWrap: 'wrap' },
    ...
    function TokenUsageSection() {
      ...
      const quotaPanel = h('div', { style: styles.quota },
        ...
        configured
          ? h('div', null,
              h('div', { style: styles.quotaRemainLabel }, '本周期剩余'),
              h('div', { style: styles.quotaRemainRow },
                h('div', { style: styles.quotaRemainValue }, formatUnits(quota.remaining)),
                h('div', { style: styles.quotaRemainMeta }, '本月额度 ' + formatUnits(quota.quotaTokens) + '（' + formatNumber(quota.quotaTokens) + '）')
              ),
